...
/** インポート先セクション コンポーネント */
export default class TargetSection extends React.Component<TargetSectionProps, TargetSectionState> {
    ...
    constructor(props: TargetSectionProps, context) {
        ...
        this.initPnPJs(props.token, `https://${props.domain}.sharepoint.com/`);
    }

    /** PnP初期化 */
    private initPnPJs(token: string, webUrl: string) {
        sp.setup({
            sp: {
                headers: {
    ...
    /** ステートにセット */
    private setToState(state: Partial<TargetSectionState>): Promise<void> {
        return new Promise<void>((resolve: () => void) => {
            this.setState(state as TargetSectionState, () => {
                resolve();
            });
        });
    }
                ...
                return this.getWebs().then(
                    (webs) => {
                        if (spoSiteSelected) {
                            ...
                            // PnP.jsを再初期化
                            this.initPnPJs(token, spoSiteSelected.key.toString());
    ...
    private handleSiteChanged = (event: React.FormEvent<IComboBox>, option?: IComboBoxOption, index?: number, value?: string) => {
        ...
        this.initPnPJs(token, newSiteUrl);
